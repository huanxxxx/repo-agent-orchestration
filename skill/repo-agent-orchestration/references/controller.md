...
Treat a wake as one bounded decision batch: consume the wake-causing report and only already-delivered facts required for the same atomic decision. End the controller turn after its synchronous accept, dispatch, correction, integration, or report actions finish. Messages or checkpoints expected in the future belong to a later wake; do not keep the turn alive to catch them.
...
After PASS, classify any proposed closeout diff as `continuity_only` or `normative` using [continuity.md](continuity.md). Keep the reviewed checkpoint separate from a later continuity checkpoint. For `continuity_only`, constrain writes to the repository-declared rolling handoff and minimal index or status pointers, verify the allowlisted diff under the current repository-root write lease, and commit without peer review. Never dispatch a docs-only reviewer to prove that PASS was recorded. If the change affects implementation, normative design or contracts, acceptance, non-goals, findings, or verdict-bearing evidence, classify it as `normative` and reopen the applicable baseline before review.
