...
11. Give every dispatched stage a non-`none` missing-report checkpoint. Use one purposeful status query only after a valid event: a due checkpoint, input request, blocker signal, user status request, or acceptance decision. Distinguish an in-progress turn from a completed turn before sending a continuation. Stop after one query if no new fact appears.
12. Never simulate a one-shot check with a recurring automation. If the product lacks a true one-shot wakeup, report that limitation.
