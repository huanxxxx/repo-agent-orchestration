...
6. Prove task execution-directory binding before granting work. A path written into the prompt is not cwd binding. Reject projectless tasks, App-created worktrees, and a repository-root `local` task when the intended target is an existing repository-local worktree. When the task API can only run a saved project locally, require a saved project whose resolved path is exactly the intended existing worktree; otherwise stop and report the capability gap.
7. Start a newly created task with a read-only coordinate check and no write authority. Verify its returned non-null project id and actual cwd against the intended worktree, then send a separate write authorization only after the binding receipt passes. Never create a replacement merely because setup is pending or a result is delayed.
...
9. Require event-driven milestone reports with the evidence required by the milestone. Deliver task decisions directly through the task-message capability; never make the user relay routine coordination. Confirm the call succeeds before claiming dispatch, authorization, or ownership transfer.
10. Use one purposeful status query only after a valid event: a due missing-report checkpoint, input request, blocker signal, user status request, or acceptance decision. Stop after one query if no new fact appears.
11. Never simulate a one-shot check with a recurring automation. If the product lacks a true one-shot wakeup, report that limitation.

Read [references/contracts.md](references/contracts.md) before creating a write task, review task, or milestone report. Run `scripts/validate_dispatch_contract.py` against the task packet before creation and against the binding receipt before granting execution authority.
