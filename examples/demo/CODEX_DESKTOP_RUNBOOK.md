...
6. Build blocked and final reports with `--task-message-to <target-task-id>`, which injects `DELIVERY` and `TARGET_SETTINGS: preserve`. Confirm the task-message call omits `model` and `thinking` and succeeds before the peer emits its local final.
