...
4. Create each task with an inert route fingerprint and no execution authority. After the creation receipt, send one validated packet containing its actual task id; the task runs the fast route gate and continues on PASS. Each command must use its exact execution path.
