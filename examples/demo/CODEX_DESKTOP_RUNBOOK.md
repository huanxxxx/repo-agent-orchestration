...
4. Create each task with an inert route fingerprint and no execution authority. After the creation receipt, run the constructor with `--task-message` and pass its exact `threadId` and raw `prompt` unchanged to `send_message_to_thread`; never hand-write a `<codex_delegation>` envelope or `source_thread_id` because App adds them. The task runs the fast route gate and continues on PASS. Each command must use its exact execution path.
