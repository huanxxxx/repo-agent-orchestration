...
4. Create each task with an inert route fingerprint and no execution authority. After the creation receipt, use `send_message_to_thread` once with the raw validated packet containing its actual task id; never hand-write a `<codex_delegation>` envelope because App adds it. The task runs the fast route gate and continues on PASS. Each command must use its exact execution path.
